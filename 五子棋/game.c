...
void Game_player(char arr[ROW][COL], int row, int col)//玩家下棋
{
	int q = 0;
	int w = 0;
	int e = 0;
...
char Right_to_left_lower(char arr[ROW][COL], int row, int col, int q, int w)//从右向左斜五个棋相同向下走
{
...
char Right_to_left_left(char arr[ROW][COL], int row, int col, int w)//从右向左斜五个棋相同向左走
{
	int q = 0;
	for (q = 0; w - 4 >= 0; q++, w--)
	{
...
char Left_to_right_lower(char arr[ROW][COL], int row, int col,int q)//从左向右斜五个棋相同向下走
{
	int w = 0;
	for (w = 0; q + 4 < row; w++, q++)
	{
...
char Left_to_right_right(char arr[ROW][COL], int row, int col, int w)//从左向右斜五个相同向右走
{
	int q = 0;
	for (q = 0;  w + 4 < col; q++, w++)
	{
...
char Win_or_lose(char arr[ROW][COL], int row, int col)//判断输赢
{
	int q = 0;
	int w = 0;
	char e = 0;
	...
	{
		e = Left_to_right_right(arr,row,col,w);//从左向右斜五个棋相同向右走
		if (e != 'T')
		{
			return e;
		}
	...
	{
		e = Left_to_right_lower(arr, ROW, COL,q);
		if (e != 'T')
		{
			return e;
		}
	...
	{
		e = Right_to_left_left(arr, ROW, COL, w);//从右向左斜五个棋相同向左走
		if (e != 'T')
		{
			return e;
		}
	...
	{
		e = Right_to_left_lower(arr, ROW, COL, q, w);//从右向左斜五个棋相同向下走
		if (e != 'T')
		{
			return e;
		}
